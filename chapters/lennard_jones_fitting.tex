...
we will need to use the mean squared error for evaluating
...
The train and test steps are wrapped as a Tensorflow function
...
We subsequently train the neural network for 50 epochs, computing
...
on test data. The mean squared error does not tell the complete story,
...
The difference in derivatives is a bit more pronounced, as the
...
as this will improve the accuracy of the derivatives
which can be potentially orders of magnitudes larger than the
error in the potential energy.
...
is an order of magnitude larger than the error in the potential energy fit.
This is expected behaviour when we do not incorporate the error
in the derivative in the loss function. However, since
this behaviour occurs only at the edge of the input space,
dynamics can also be preserved by training on a representative set
...
and will cause problems regardless of the accuracy of the
interatomic potential.

Write something about the root mean squared errors...
